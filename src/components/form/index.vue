...
const tabs = ref([
  {
    ...
    formItem: {
      userLabel: '小米账号：',
      userPlaceholder: '请输入小米运动账号',
      passwordLabel: '小米密码：',
      passwordPlaceholder: '请输入小米运动密码',
      stepLabel: '运动步数：',
    },
  },
  {
  ...
  <div class="pr-4 py-5">
    ...
    >
      <el-form-item :label="tabs[current].formItem.userLabel" prop="mobile">
        <el-input
          v-model.trim="tabs[current].form.mobile"
          :placeholder="tabs[current].formItem.userPlaceholder"
          clearable
        />
      </el-form-item>

      <el-form-item
        v-if="tabs[current].password"
        :label="tabs[current].formItem.passwordLabel"
        prop="password"
      >
        <el-input
          v-model.trim="tabs[current].form.password"
          type="password"
          clearable
          show-password
          :placeholder="tabs[current].formItem.passwordPlaceholder"
        />
      </el-form-item>

      <el-form-item :label="tabs[current].formItem.stepLabel" prop="step">
        <el-input
          v-model.trim="tabs[current].form.step"
      ...
      <el-form-item>
        <el-row class="w-full m-0" justify="space-between">
          <el-col :span="8">
            <el-button @click="resetForm(ruleFormRef)" class="w-full"
              >重置</el-button
            >
          </el-col>
